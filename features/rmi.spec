...
Feature: silo rmi — Remove the workspace image
  ...
  Rule: Removes the workspace image
    ...
    Scenario: rmi removes the workspace image
  ...
  Rule: Requires workspace to be initialized
    ...
    Scenario: rmi fails when workspace is not initialized
      Given a clean workspace with no existing silo files
      When I run `silo rmi`
      Then the exit code should not be 0
      And the error should indicate ".silo/silo.toml" is missing

  Rule: rmi does not remove the user image

    Scenario: rmi only removes the workspace image, not the user image
      Given the workspace image "silo-abc12345" exists
