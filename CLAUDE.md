...
- `config.go` — Configuration management (TOML-based, two-tier: global + workspace) and workspace initialization (`initWorkspaceConfig`)
...
1. Built-in defaults
2. Global config at `$XDG_CONFIG_HOME/silo/`
...
**Devcontainer merge:** `silo devcontainer` merges `$XDG_CONFIG_HOME/silo/devcontainer.json` into the generated output — objects merge recursively (key-by-key), arrays concatenate (base first), scalars from overlay win.
...
- `setupGlobalConfig(t)` — sets `XDG_CONFIG_HOME` to a temp dir with scaffold files
